...
def sch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(module, "NODE_PROBE_CACHE_FILE", state_dir / "node_probe_cache.json", raising=False)
    monkeypatch.setattr(module, "LOCK_FILE", state_dir / ".lock", raising=False)
    monkeypatch.setattr(module, "WATCHER_LOCK_FILE", state_dir / ".watcher.lock", raising=False)
    monkeypatch.setattr(module, "LAUNCH_EXEC_LOCK_FILE", state_dir / ".launch_exec.lock", raising=False)
    monkeypatch.setattr(module, "DISPATCH_INTENT_FILE", state_dir / ".dispatch_intent.json", raising=False)
