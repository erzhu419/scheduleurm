#!/usr/bin/env python3
"""Figure 1 combo candidate: v1 quadrant panel plus tightened v2 ledger/gate.
...
a tightened ledger/gate view aligned with the manuscript's theorem objects.
"""
...
def panel_b(ax):
    V2.clean(ax)
    V2.label(ax, "b", x=0.0)
    ax.text(0.03, 0.95, "Measured-service action ledger", ha="left", va="top", fontsize=8.3, fontweight="bold", color=COL["ink"])
    ax.text(0.03, 0.895, "one lower-service table for candidate and diagnostic rows", ha="left", va="top", fontsize=5.4, color=COL["muted"])

    x0, y0, w, h = 0.035, 0.15, 0.93, 0.66
    ...
    rows = [
        ("declared\nq00-q11", 0.72, 0.18, "0 / audit", "admit", COL["green"]),
        ("candidate\ncover", 0.64, 0.24, "audit", "admit", COL["blue"]),
    ...
    ax.text(0.50, 0.065, "External rows audit the action family; unknown states receive zero theorem service until profiled.", ha="center", va="center", fontsize=5.25, color=COL["muted"])


def panel_c(ax):
    V2.clean(ax)
    V2.label(ax, "c", x=0.0)
    ax.text(0.03, 0.95, "Slack gate to theorem-facing claims", ha="left", va="top", fontsize=8.4, fontweight="bold", color=COL["ink"])
    ...
    V2.round_box(ax, (0.375, 0.235), (0.25, 0.095), "trace/audit\nlive hook", fc=COL["panel"], ec=COL["hair"], fs=5.35)
    V2.round_box(ax, (0.68, 0.235), (0.25, 0.095), "not claimed\nfuture/SOTA", fc=COL["red_light"], ec=COL["red"], fs=5.35)
    ...
    ax.text(0.50, 0.105, "Additive constants set the finite-set threshold; slope losses set eta.", ha="center", va="center", fontsize=5.25, color=COL["muted"])


def main():
    fig = plt.figure(figsize=(7.35, 4.55), constrained_layout=False)
    gs = fig.add_gridspec(2, 2, width_ratios=[1.18, 1.0], height_ratios=[1, 1], wspace=0.045, hspace=0.20)
