...
| Direct full-stack SOTA superiority gate | `algorithm/experiments/sota_fullstack_superiority_gate.py` | Converts full-stack superiority into an executable reviewer gate.  Current output is `direct_fullstack_named_sota_superiority_ready=true`, `full_stack_ready_count=5`, and `named_sota_fullstack_superiority_count=5` for Gavel, Pollux/AdaptDL, Sia, IADeep, and Salus on scoped same-host same-workload probes. |
| SOTA universe registry gate | `algorithm/experiments/sota_universe_registry_gate.py` | Separates the closed named-five same-host same-workload claim from registered or arbitrary SOTA-universe language.  The current gate keeps `registered_sota_universe_superiority_ready=false` and `arbitrary_sota_superiority_ready=false` until comparable full-stack rows exist for every registered external system. |
...
| Multi-node original deployment gate | `algorithm/experiments/multinode_original_deployment_gate.py` | Keeps same-host full-stack evidence separate from original multi-node deployments.  Current read-only SSH inventory sees two reachable GPU nodes (`jtl110gpu`, `jtl110gpu2`) but does not promote a claim; multi-node superiority needs per-system original control-plane/worker runs. |
...
direct SOTA adapters:
  ...
  SOTA candidate-action union closes the measured-cache policy-family envelope claim, while the separate strict full-stack gate closes named-system same-host same-workload superiority;
  ...
  strict full-stack superiority gate reports full_stack_ready_count=5 and named_sota_fullstack_superiority_count=5 for Gavel/Pollux/AdaptDL/Sia/IADeep/Salus measured probes.
