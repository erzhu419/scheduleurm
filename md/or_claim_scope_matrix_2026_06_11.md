...
Named external runtime-probe gate: md/sota_fullstack_superiority_gate_20260613.md NAMED 5/5 PASS; named_same_host_runtime_probe_ready=true, direct_fullstack_named_sota_superiority_ready=false, with Gavel physical, Pollux/AdaptDL, Sia, IADeep, and Salus scoped same-host same-workload runtime-probe rows ready and paired native-better evidence on the measured probes; do not generalize this to arbitrary systems, future workloads, multi-node original deployments, or production-wide organic traces
SOTA universe registry gate: md/sota_universe_registry_gate_20260614.md PASS for named five runtime-probe scope and registered-system inventory; registered_sota_universe_superiority_ready=false and arbitrary_sota_superiority_ready=false
